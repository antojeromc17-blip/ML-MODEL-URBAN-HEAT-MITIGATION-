"""
Complete Autonomous Mumbai Pipeline
===================================
...
4. Generation of data/mumbai/grid_output.geojson
"""
import os
import json
import time
...
GRID_SIZE_M = 500
UTM_EPSG = 32643  # UTM zone 43N for Mumbai (~72.85°E, 19.0°N)

BASE_DIR = Path(__file__).resolve().parent
DATA_DIR = BASE_DIR / "data" / "mumbai"
MODEL_DIR = BASE_DIR / "model" / "mumbai"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

LST_TIF = DATA_DIR / "mumbai_lst.tif"
NDVI_TIF = DATA_DIR / "mumbai_ndvi.tif"
NDBI_TIF = DATA_DIR / "mumbai_ndbi.tif"
BUILDINGS_GEOJSON = DATA_DIR / "mumbai_buildings.geojson"
ROADS_GEOJSON = DATA_DIR / "mumbai_roads.geojson"

FEATURES_CSV = DATA_DIR / "mumbai_grid_features.csv"
GRID_GEOJSON = DATA_DIR / "mumbai_grid.geojson"
...
def create_grid(west, south, east, north, cell_size_m):
    print(f"Creating {cell_size_m}m x {cell_size_m}m grid for Mumbai...")
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("STARTING MUMBAI PIPELINE")
    print("=" * 60)
    ...
    lst_std = df_valid["lst_std"].std() if "lst_std" in df_valid else df_valid["lst_mean"].std()
    ...
    print("\nAssembling Mumbai grid_output.geojson...")
    ...
    print(f"\nSaved Mumbai GeoJSON: {GRID_OUTPUT_GEOJSON} ({file_size_mb:.2f} MB)")
    print(f"Total pipeline time: {time.time() - t0:.1f}s")
    print("MUMBAI PIPELINE COMPLETE!")
